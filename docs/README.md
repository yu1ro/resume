...
2023年12月現在8店舗の運営に利用され、店舗を増やす計画を進められる程度にはスムーズに運用されています。
...
レビュー業務を効率化するために、GitHub Actionsでlint等の自動チェックを行う仕組みを整えました。
メンバー間でFlutterのバージョンが揃うようにasdfを組み込む仕組みを整えました。
Unit Test / Widget Testを行う基盤を整えました。

バックエンド/Webフロントエンドの進行が遅れた時にはバックエンドのタスクも手伝うことがあります。

アプリ/バックエンドの両方を担当したことで、視野広く活動できています。
柔軟に動けるメンバーとなることで、人件費の最適化に貢献できていると考えています。
...
前プロジェクトで部署間の調整業務を行っていたこともあり、
他の部署を巻き込んだ機能の仕様策定・調整業務も滞り無く進行することができました。
...
各アプリで利用しているAPIごとに徐々にリプレイスしており、
ジョインした時点で一部のアプリでリプレイスが既に完了していたため、運用と開発も平行して行われていました。

**担当業務**
インターフェイスを保ちつつ同じ動きを担保するために、差分の洗い出しと修正業務を行いました。
また実際の切り替えを行う際の他プロジェクトとの調整業務を行いました。
予定していたリプレイスが完了したことで、プロジェクトが解体・縮小されました。
...
**担当業務**
Androidエンジニアとして新卒配属。
ロードマップ上の機能開発を期限通りに実装できました。
他にもアプリレビュー向上担当として、レビューへの返信業務も行うことで、ユーザー目線で機能検討に加わることができました。
ユーザーからの声を感じることで、QAに依頼する前からの品質が向上しました。
...
- アプリだけではなくバックエンド・インフラなどへの興味関心があり、学習しながらもアウトプット可能です。
- 型システムが厳密な言語が得意です。(逆にいうとPHPやJSは苦手)
